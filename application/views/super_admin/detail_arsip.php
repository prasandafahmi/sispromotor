...
?>
            <form action="<?php echo site_url(); ?>/super_admin/Pemesanan/validasi/<?php echo $p->id_detail;?>" method="post"> 
              <tr>   
        ...
        <tr> 
          ...
          <?php if($p->aturan_lain==""){echo ""; }else{?>  <td><?php echo $p->aturan_lain; ?></td><?php }?>
            <td>
                ...
                <?php }
                $p=$produk->row();
                $tem=$this->db->query("select *, pembayaran.status as sp from `order` join detail on detail.id_order=`order`.id_order join pembayaran on pembayaran.id_order=`order`.id_order where pembayaran.id_order=".$p->id_order)->row();
                ?>
                
                <tr>
                    <th>Status Pembayaran</th>
                    <td><?php if($tem->harga==0){echo "Gratis";}else{echo "Bayar";}?></td>
                </tr>
                 <tr>
                    <th>Status Validasi</th>
                    <td><?php echo $tem->sp;?></td>
                </tr>
                      <tr>
                        
                         <td colspan="3"> <a href="<?php echo site_url();?>/super_admin/Apemesanan">Kembali</a>
                         </td>
