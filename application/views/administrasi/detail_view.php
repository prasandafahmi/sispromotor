                     ...
                     <h4 class="modal-title">Pemesanan</h4>
                      <br>
                        <div class="table-responsive">
                         <?php if(!empty($id)) { $p=$produk->result();?>
                         <table class="table table-striped table-advance table-hover">
                            
                                 <?php foreach($produk->result() as $p){
    
?>      <form action="<?php echo site_url(); ?>/administrasi/Pemesanan2/validasi/"  method="post"> 
                             <tr>   
        ...
        <tr> 
         ...
         <?php if($p->alamat_kirim==""){echo ""; }else{?>  <td><?php echo $p->alamat_kirim; ?></td><?php }?>
            ...
            <td>
                <?php if($p->pelaksana!=""){ ?>
        <input type="text" class="form-control" id="pelaksana" placeholder="pelaksana" value="<?php echo $p->pelaksana;?>" name="f1" disabled>
                <?php }else{ ?> 
            <input type="text" class="form-control" id="pelaksana" placeholder="pelaksana"  name="f1" required>
                <?php } ?>
            <input type="hidden" class="form-control" id="pelaksana" value="<?php echo $p->id_detail;?>" name="id_detail">
            <input type="hidden" class="form-control" id="pelaksana" value="<?php echo $p->id_order;?>" name="id_order"></td>
        </tr> 
      
                    <tr>
                         <?php if(! empty($p->pelaksana)){?>
                         <td colspan="3"> <a href="<?php echo site_url('administrasi/Pemesanan2');?>">Kembali</a>
                         </td>
                         <?php }else{?>
