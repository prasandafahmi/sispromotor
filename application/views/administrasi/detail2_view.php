...
?>     
              <form action="<?php echo site_url(); ?>/administrasi/Result/update_akhir/" method="post" enctype="multipart/form-data">
                <tr>   
                   ...
                   <?php if($p->result==""){echo ""; }else{?>  <th>Result</th><?php }?>
        ...
        <tr> 
          ...
          <td><?php if(! empty($p->result)){?><a href="<?php echo site_url()?>/administrasi/Result/download_hasil/<?php echo $p->result; ?>"class="btn btn-primary btn-sm tebal"><span class="icon_download "></span> Download result</a><?php }?></td>
          ...
          <?php if($p->tgl_penggunaan==""){echo ""; }else{?>  <td><?php echo $p->tgl_penggunaan; ?></td><?php }?>
            <td> <input type="file" id="exampleInputFile3" name="f2" required>
            <input type="hidden" name="id" value="<?php echo $p->id_detail;?>">
            <input type="hidden" name="ido" value="<?php echo $p->id_order;?>">
            </td>
        </tr>
            <?php if($p->kategori=="Pustaka"){?>
                  <div class="row">
                      <div class="col-sm-3"><label>No Resi</label></div>
                      <div class="col-sm-8"><input type="text" class="form-control" name="resi" required><br>
                      </div>
           <div class="col-sm-3"><label>Ekspedisi Pengiriman</label></div>
                      <div class="col-sm-8"><select class="form-control" name="expedisi" required>
                <option >POS Indonesia</option>
                <option >JNE</option>
                <option >Tiki</option>
                </select><br></div>
            <div class="col-sm-3"><label>Atas Nama</label></div>
                      <div class="col-sm-8"><input type="text" class="form-control" name="namakirim" required><br></div>
                  </div>
            <?php }?>       
        <tr>
           <?php if(! empty($p->status_akhir)){?>
          <td> 
              <a href="<?php echo site_url();?>/administrasi/Result">Kembali</a>
          </td>
         <?php }else{?>
         <td>  
             <button>Kirim</button>
